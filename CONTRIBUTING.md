...
There are two systems for procedurally generating tests. To generate/regenerate all generated tests, run these two commands:

```
./make.py
./tools/generation/generate_type_coercion.py
```

Invoke the tools with `--help` to get CLI help.

### Case and template tests
...
The abstract operations for type coercion found in the spec such as `ToIndex` and `ToBoolean` have complex behavior and are frequently overlooked when writing tests for builtin functions that call them. The type coercion test generator assists in generating thorough tests for these type coercion abstract operations called through a builtin function.

Tests are generated by searching `src/type-coercion` recursively for `*.py` files. Each file must be a module that defines a function with the following name and signature:
...
The `generate_type_coercion` function is called once with the parameter `generate_tests` which is a function that can be called multiple times to generate tests.

The `generate_tests` function only accepts keyword arguments in the form `name=value`, not positional arguments. These are the keyword arguments for `generate_tests`:

Name | Description
-----|------------
`path_prefix` | *required*. The path relative to the output dir where tests will be generated. Do not include the file extension. e.g. `"built-ins/BigInt/asIntN/bits-toindex"`.
`frontmatter` | *required*. A `dict` used to derive the yaml frontmatter of the generated tests. See below.
`conversion` | *required*. One of the supported conversion functions, such as `"ToString"` or `"ToBigInt"`. See below.
`templates` | *required*. A `dict` defining code templates. An example entry might be `'0': 'assert.sameValue("a".indexOf("a", %(value)s), 0, %(message)s);'`. See below.
`nominal_value_cases` | *conditionally required*. A `list` of `(value, template)` pairs. This is required for some `conversion` functions, and disallowed otherwise. See below.
`preamble` | *optional*. A `str` of JavaScript code that will be emitted at the start of every test file. Use this to initialize global variables, if necessary.

#### `frontmatter`

Only these frontmatter fields are currently allowed in the `frontmatter` parameter: `"description"` (required), `"esid"`, `"info"`, `"includes"` (`list` of `str`), `"features"` (`list` of `str`).

If a frontmatter field is a `str` that contains `"\n"` characters, each non-whitespace line of the string may be indented. The indentation of the first line will be removed from all lines. It is an error for a non-whitespace line to be indented less than the first line.

#### `conversion`, `templates`, `nominal_value_cases`

The set of required/allowed keys of the `templates` dict vary depending on the `conversion` function being tested. See below.

The values of the `templates` dict and the second value of each nominal value case pair are JavaScript code templates. They will be evaluated with Python's [%-style str formatting](https://docs.python.org/2/library/stdtypes.html#string-formatting) with named parameters. Parameters named `"value"` and `"message"` will always be given. Put `%(value)s` in your code where the value should go. More named parameters may be defined as described below.

When you call an assert function, such as `assert.sameValue`, include `%(message)s` as the final parameter. If a message is not required for a particular generated case, the string `", %(message)s"` will be removed from the template before evaluating it.

If a template is required with the key `"throws"`, it will be used to generate tests which are expected to throw. A template parameter named `"error"` will be provided, which is the expected exception type. The template should probably look something like this: `'assert.throws(%(error)s, function() { "".indexOf("", %(value)s); }, %(message)s);'`

If a nominal value case expects an `int`, a `long` is also acceptable.

This is the list of currently supported `conversion` functions:

* `"ToBoolean"`: `templates` must define `'false'` and `'true'` templates. `nominal_value_cases` must not be specified.
* `"ToInteger"`: `templates` must define `'0'`, `'1'`, `'Infinity'`, and `"throws"`. `nominal_value_cases` must be a list of `(int, str)` pairs. For each pair, the `int` will be used to generate a few cases, such as appending `.9` to assert that the value is truncated towards 0.
* `"ToIndex"`: `templates` must define `'0'`, `'1'`, and `"throws"`. `nominal_value_cases` must be a list of `(int, str)` pairs as with `"ToInteger"` above.
* `"ToBigInt"`: `templates` must define `'0n'`, `1n`, and `"throws"`. `nominal_value_cases` must be a list of `(int, str)` pairs. It is recommended for one of these pairs to contain an integer larger than `Number.MAX_SAFE_INTEGER`, such as `12345678901234567890`. The provided frontmatter features must include `"BigInt"`.
* `"ToString"`: `templates` must define `"throws"`. Additionally `templates` *may* define cases for specific strings, such as `''`. If a template is not defined for the expected value of a test, then a template with the key `str` is used and passed an additional template parameter `"expected_string_contents"`. The value of `"expected_string_contents"` does not include surrounding quotes, and is guaranteed to be usable by naively surrounding the string in double quotes. An example of using this template is: `'assert.sameValue("__%(expected_string_contents)s__".indexOf(%(value)s), 2, %(message)s);'`. Note that the key in the `templates` dict should be the `str` builtin Python object, not the string `"str"`. This is to avoid ambiguity with providing a template for a specific case.
