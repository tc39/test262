...
function EveryShrinkMidIteration() {
  ...
  for (let ctor of ctors) {
    ...
    assert(ArrayEveryHelper(fixedLength, CollectValuesAndResize));
  ...
  for (let ctor of ctors) {
    ...
    resizeAfter = 1;
    resizeTo = 3 * ctor.BYTES_PER_ELEMENT;
    assert(ArrayEveryHelper(fixedLengthWithOffset, CollectValuesAndResize));
  ...
  for (let ctor of ctors) {
    ...
    resizeAfter = 2;
    resizeTo = 3 * ctor.BYTES_PER_ELEMENT;
    assert(ArrayEveryHelper(lengthTracking, CollectValuesAndResize));
    assert.compareArray(values, [
      0,
      2,
      4
  ...
  for (let ctor of ctors) {
    ...
    resizeAfter = 1;
    resizeTo = 3 * ctor.BYTES_PER_ELEMENT;
    assert(ArrayEveryHelper(lengthTrackingWithOffset, CollectValuesAndResize));
    assert.compareArray(values, [4]);
  }
}
